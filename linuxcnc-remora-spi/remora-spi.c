...
void spi_read()
{
	...
	if (*(data->SPIenable))
	{
		// Transfer every servo period so the RP2350 DMA sees a continuous stream of
		// 64-byte packets rather than only on SPIreset rising edge.
		{
			static int pi_print_count = 0;
			if (pi_print_count++ < 3) {
				int _i;
				rtapi_print("txBuf[0..63]:\n");
				for (_i = 0; _i < 64; _i += 8) {
					rtapi_print("  [%02d] %02x %02x %02x %02x %02x %02x %02x %02x\n",
						_i,
						txData.txBuffer[_i+0], txData.txBuffer[_i+1],
						txData.txBuffer[_i+2], txData.txBuffer[_i+3],
						txData.txBuffer[_i+4], txData.txBuffer[_i+5],
						txData.txBuffer[_i+6], txData.txBuffer[_i+7]);
				}
			}
		}
		spi_transfer();
