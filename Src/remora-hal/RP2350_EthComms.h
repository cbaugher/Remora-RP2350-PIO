...
public:
    ...
    uint8_t write_byte(uint8_t b)  override;

    // W5500 driver calls these for bulk socket buffer transfers
    void DMA_write(uint8_t* data, uint16_t len) override;
    void DMA_read (uint8_t* data, uint16_t len) override;

    // Called by W5500 UDP receive callback when a valid packet arrives
    void flag_new_data() override;
};
